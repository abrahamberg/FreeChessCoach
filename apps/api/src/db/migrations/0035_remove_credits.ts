 ...
 * - Migrates any `paused_no_credits` sessions/puzzle_sessions to `abandoned`
 *   and drops that status value from both tables' status CHECKs (the app can
 *   no longer pause a session for lack of credits — without a platform key
 *   there's nothing to run out of). `puzzle_sessions` (0029_puzzle_sessions.ts)
 *   post-dates the original credit system but reused its status vocabulary,
...
export async function up(db: Kysely<unknown>): Promise<void> {
  ...
  `.execute(db);
  await sql`UPDATE puzzle_sessions SET status = 'abandoned' WHERE status = 'paused_no_credits'`.execute(db);
  await sql`ALTER TABLE puzzle_sessions DROP CONSTRAINT puzzle_sessions_status_check`.execute(db);
  await sql`
    ALTER TABLE puzzle_sessions ADD CONSTRAINT puzzle_sessions_status_check
      CHECK (status IN ('active','completed','abandoned'))
  `.execute(db);
