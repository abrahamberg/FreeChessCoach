...
if (changed.length === 0) {
  console.log('No packages changed, skipping lint');
  process.exit(0);
}

console.log(`Linting changed packages: ${changed.map(p => p.name).join(', ')}`);

for (const pkg of changed) {
  if (!existsSync(`${pkg.dir}/package.json`)) continue;
  console.log(`\n--- Linting ${pkg.name} ---`);
  try {
    execSync(`npm run lint -w ${pkg.name}`, { stdio: 'inherit', cwd: process.cwd() });
  } catch (e) {
...
console.log('\nAll changed package lint passed!');