...
describe('ms01OpponentCheckScanOmission', () => {
  test('fires when the opponent has a check available after the move', () => {
    const ctx = buildPlyDiagnosticContext(moveTo(OPPONENT_HAS_CHECK_CAPTURE_THREAT_FEN))!;

    const observation = ms01OpponentCheckScanOmission.detect(ctx);

    expect(observation).not.toBeNull();
    expect(observation!.code).toBe('MS-01');
    expect(observation!.direction).toBe('D');
    expect(observation!.failed).toBe(true);
  });

  test('does not fire when the opponent has no check available', () => {
    const ctx = buildPlyDiagnosticContext(moveTo(NO_OPPONENT_OPPORTUNITIES_FEN, { quality: 'blunder', drop: 40 }))!;

    expect(ms01OpponentCheckScanOmission.detect(ctx)).toBeNull();
  });

  test('does not suppress MS-02 firing on the same ply', () => {
    const ctx = buildPlyDiagnosticContext(moveTo(OPPONENT_HAS_CHECK_CAPTURE_THREAT_FEN))!;
