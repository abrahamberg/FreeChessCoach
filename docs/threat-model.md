...
- **T4** — `plugins/route-rate-limit.ts`: per-user fixed-window limits (setup probe/save 10/min, unlock 10/5 min, engine analyze/hint 120/min, ping 10/min, remote game lists 30/min). It's a plain `preHandler`: `@fastify/rate-limit` was tried and its limits **silently never applied**, because its `onRoute` hook loads after `buildApp` registers routes. The test caught it. Key derivation uses async `scrypt`.
